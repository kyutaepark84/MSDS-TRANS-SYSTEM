...
# 이 문서(폭발·화재)를 못 찾으면 split_sections()가 그 지점에서 멈춰버려서
# 뒤따르는 6~16번 항목이 통째로 빈 값이 되므로, SEP를 쓰는 자리는 모두
# "및"도 허용한다.
_SEP_OR_AND = rf"(?:{SEP}|및)?"

SECTION_TITLE_PATTERNS = {
    1: r"화학제품과\s*회사에\s*관한\s*정보",
    2: rf"유해성?\s*{_SEP_OR_AND}\s*위험성",
    3: r"구성\s*성분(?:의|과)?\s*명칭\s*및\s*함유량",  # "구성성분과 명칭..."처럼 조사가 다른 문서도 있음
    4: r"응급\s*조치\s*요령",
    5: rf"폭발\s*{_SEP_OR_AND}\s*화재\s*시\s*대처\s*방법",
    ...
    9: rf"물리\s*{_SEP_OR_AND}\s*화학적\s*특(?:성|징)",
...
_CAS_RE = re.compile(r"\d{2,7}\s*[-–]\s*\d{2}\s*[-–]\s*\d")


def _normalize_cas(raw_cas):
    return re.sub(r"\s*[-–]\s*", "-", raw_cas)
...
STOP = (
    rf"(?=(?<![가-힣])[{_ORDINAL_CHARS}]\.\s|(?<![가-힣])[{_ORDINAL_CHARS}]\)|\(\d+\)|\d+\)|\d+(?:\.\d+)+\.?|-\s+\S|○\s*\S|\Z)"
)
_STOP_MATCH = rf"(?<![가-힣])[{_ORDINAL_CHARS}]\.\s|(?<![가-힣])[{_ORDINAL_CHARS}]\)|\(\d+\)|\d+\)|\d+(?:\.\d+)+\.?|-\s+\S|○\s*\S"
...
# 잡아낸다.
_HEADER_BLOCK_TOKEN = (
    r"(?:MSDS\s*번호\s*[:：]?\s*\S+|물\s*질\s*안\s*전\s*보\s*건\s*자\s*료|"
    r"\(Material Safety Data Sheets\)|문서번호\s*[:：]?\s*\S+|"
    r"개정번호\s*[:：]?\s*\S+|개정일자\s*[:：]?\s*[\d.]+)"
)
_HEADER_BLOCK_RE = rf"(?:{_HEADER_BLOCK_TOKEN}\s*){{2,}}"
...
_BOILERPLATE_PATTERNS = [
    ...
    r"포함된\s*물질[^./]*있음\.?",
    r"\S+\(MSDS\)\s+\d+/\d+",  # 쪽 하단 "제품명(MSDS)   2/6" 식 꼬리말
]
...
FIREFIGHTING_LABELS = {
    ...
    "hazards": r"화학물질로부터\s*생기는\s*특정\s*유해성(?:\s*\([^)]*\))?",
...
HANDLING_STORAGE_LABELS = {
    ...
    "storage": r"(?:안전한\s*저장\s*방법|보관\s*방법)(?:\s*\([^)]*\))?",
}
PPE_LABELS = {
...
def extract_flat_text(pdf_path):
    ...
    flat = re.sub(r"([가-힣]+시)\s+오(?![가-힣])", r"\1오", flat)
...
def _parse_classification(section2):
    ...
    if re.search(r"화학물질의\s*분류", body):
        names = [
            m.group(1).strip()
            for m in re.finditer(r"화학물질의\s*분류\s*[:：]?\s*(.+?)\s*(?=\d+\)|\Z)", body)
        ]
        grades = [
            m.group(1).replace(" ", "")
            for m in re.finditer(
                rf"유해성?{SEP}?\s*위험성\s*구분\s*[:：]?\s*((?:만성)?구분\s*\d+|해당\s*없음)", body
            )
        ]
        pairs = [
            (name, grade) for name, grade in zip(names, grades)
            if name and grade and "해당" not in grade
        ]
        if pairs:
            return pairs[:15]
    pairs = []
...
def _extract_coded_statements(text, code_re, max_chars=150):
    ...
    for i, m in enumerate(matches):
        ...
        # 그룹 표제 문구("예방조치문구", "유해ㆍ위험문구" 등)는 실제로 소항목
        # 표제로 쓰일 때만 경계로 삼아야 한다 — "모든 안전 예방조치 문구를
        # 읽고 이해하기 전에는..."처럼 같은 글자가 일반 문장 속에서 조사와
        # 함께 이어지는 경우까지 경계로 오인하면 그 문장이 중간에서 잘린다.
        # 표제로 쓰일 때는 뒤에 공백/콜론/다음 코드가 오고, 문장 속에서는
        # 뒤에 조사(한글)가 바로 붙으므로 그 차이로 구분한다. 한글 순서
        # 마커가 "다)" 대신 "다."(마침표) 표기인 경우도 경계로 인정한다
        # (하위 소항목이 다른 내용과 뒤섞여 추출되는 문서에서 필요).
        marker_m = re.search(
            rf"(?:(?<![가-힣])[{_ORDINAL_CHARS}]\)|(?<![가-힣])[{_ORDINAL_CHARS}]\.\s|\d+\)|\d+(?:\.\d+)+|[HP]\d{{3}}|"
            rf"예방조치\s*문구(?![가-힣])|응급조치요령(?![가-힣])|유해{SEP}?\s*위험\s*문구(?![가-힣])|<[가-힣]+>|○)", window
        )
        ...
        # 소항목 경계(marker_m)에서 잘라낸 경우, 원본에 마침표가 아예 없이
        # (예: "...폐기하시오" 바로 뒤에 다른 항목 "다."가 이어지는 문서)
        # 다음 내용이 시작되는 지점이라 desc 끝에 마침표가 안 남는다. 문장이
        # 온전한 서술형으로 끝나는데 마침표만 빠진 경우이므로 붙여 완성한다.
...
def _extract_composition_name(before):
    ...
    if not first_m:
        return ""
    name = first_m.group(0)
    is_korean = bool(re.match(r"^[가-힣]", name))
    ...
    # 그대로 이름의 일부로 이어붙인다.
    paren_m = re.match(r"\([^()]{1,40}\)", rest)
    if paren_m:
        name += paren_m.group(0)
...
def _parse_composition_reversed(section3):
    ...
    # \S+ 대신 한글 전용으로 좁힌다.
    pattern = re.compile(
        r"[가-힣]+\s+(.+?)\s*(\d[\d.]*(?:\s*~\s*\d[\d.]*)?)\s*%\s*(\d{2,7}\s*[-–]\s*\d{2}\s*[-–]\s*\d|혼합물|자료없음)"
    )
    for m in pattern.finditer(section3):
        name, content, cas = m.groups()
        name = name.strip()
        content = content.replace(" ", "")
        if cas not in ("혼합물", "자료없음"):
...
def _parse_composition(section3, product_name=""):
    section3 = re.sub(r"구성\s*성분(?:의|과)?\s*명칭\s*및\s*함유량", "", section3)
    ...
    section3 = re.sub(
        r"화\s*학\s*물\s*질\s*명(?:\s*이\s*명)?|물질명|관용명(?:\s*및\s*이명)?|이명\s*\(관용명\)|이\s*명",
        "", section3,
    )
    ...
    for i, m in enumerate(cas_matches):
        ...
        cas = _normalize_cas(m.group(0))
        if not name or name in _COMPOSITION_HEADER_NOISE:
            name = KNOWN_CAS_NAMES.get(cas, name)
